...
import { prettifyFilename, hostnameFromUrl } from "../lib/parsing";
import { SAMPLE_TEXT } from "../lib/sampleText";

// Frictionless Importer: paste / link / upload tabs, plus a one-click
// sample article. URL import proxies through Jina AI's reader endpoint
// since most sites block direct cross-origin fetches.
  ...
  return (
    <div className="cr-fade-up" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 20px" }}>
      <div style={{ width: "100%", maxWidth: 560 }}>
        ...
        <div role="tablist" style={{ display: "flex", gap: 6, background: PALETTE.paperSoft, borderRadius: 999, padding: 5, marginBottom: 22 }}>
          <button role="tab" aria-selected={tab === "paste"} className="cr-no-select" style={tabBtnStyle(tab === "paste")} onClick={() => setTab("paste")}>Paste</button>
          <button role="tab" aria-selected={tab === "url"} className="cr-no-select" style={tabBtnStyle(tab === "url")} onClick={() => setTab("url")}>Link</button>
          <button role="tab" aria-selected={tab === "upload"} className="cr-no-select" style={tabBtnStyle(tab === "upload")} onClick={() => setTab("upload")}>Upload</button>
        </div>
        ...
        {tab === "url" && (
          <div>
            ...
            {fetchError && <p role="alert" style={{ fontFamily: FONT_SANS, fontSize: 13, color: PALETTE.clay, marginTop: 10, lineHeight: 1.5 }}>{fetchError}</p>}
        ...
        {tab === "upload" && (
          <div>
            ...
            {fileError && <p role="alert" style={{ fontFamily: FONT_SANS, fontSize: 13, color: PALETTE.clay, marginTop: 10 }}>{fileError}</p>}
          </div>
        )}
